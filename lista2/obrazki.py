#Poprawione, działa - pokazać
import numpy as np

explore = 0.05
maxloops = lambda w, h: w*h*8
...
def nonogram(rows, columns, init=None):
    ...
    nrows = len(rows)
    if init is None:
        image = np.array([np.array([np.random.choice([1]) for _ in range(ncols)]) for _ in range(nrows)]) # image[y][x]
    else:
        image = init
...
# print(multi_opt_dist([0,1,1,0,1,0,1,1,0,1,0,1,0,0,1], [2,3,2]))
